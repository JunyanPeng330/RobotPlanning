...
class ReplayBuffer:
    def __init__(self, max_len, state_dim, action_dim, if_on_policy, if_gpu):
        ...
        :int state_dim: the dimension of state, 9
        :int action_dim: the dimension of action (action_dim==1 for discrete action), 6
    ...
    def sample_batch(self, batch_size) -> tuple:
        ...
        """
        indices = torch.randint(self.now_len - 1, size=(batch_size,), device=self.device) if self.if_gpu else rd.randint(self.now_len - 1, size=batch_size)
...
class Actor(nn.Module):  # DPG: Deterministic Policy Gradient
    ...
    def get_action(self, state, action_std):
        ...
        noise = (torch.randn_like(action) * action_std).clamp(-0.1, 0.1)
...
class CriticTwin(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim, if_use_dn=False):
        ...
        if if_use_dn:  # use DenseNet (DenseNet has both shallow and deep linear layer)
            nn_dense = torchvision.models.densenet.DenseNet(mid_dim)
            lay_dim = nn_dense.out_dim
            self.net_sa = nn.Sequential(nn.Linear(state_dim + action_dim, mid_dim), nn.ReLU(), nn_dense, )  # state-action value function
        else:  # use a simple network for actor. Deeper network does not mean better performance in RL.
            lay_dim = mid_dim
            self.net_sa = nn.Sequential(nn.Linear(state_dim + action_dim, mid_dim), nn.ReLU(), nn.Linear(mid_dim, lay_dim), nn.ReLU())
    ...
    def update_net(self, buffer, target_step, batch_size, repeat_times) -> (float, float):
        ...
        for i in range(int(target_step * repeat_times)):
            ...
            with torch.no_grad():
                ...
                next_a = self.act_target.get_action(next_s, self.policy_noise)  # policy noise, take the next action
                next_q = torch.min(*self.cri_target.get_q1_q2(next_s, next_a))  # twin critics, take the smaller one
            ...
            los_q = obj_critic.cpu()
            los_q = los_q.detach().numpy()
            los_pi = obj_actor.cpu()
            los_pi = los_pi.detach().cpu()

        return los_q.item(), los_pi.item() / 2